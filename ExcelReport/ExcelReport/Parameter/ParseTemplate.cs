...
namespace XmlGenerator
{
    internal static class ParseTemplate
    {
        ...
        public static Sheet2ParameterDictionary ParseToDictionary(IWorkbook workbook)
        {
            Sheet2ParameterDictionary dic = new Sheet2ParameterDictionary(workbook);
                ...
                foreach (IRow row in sheet)
                {
                    foreach (ICell cell in row.Cells)
                    {
                        if (cell.CellType.Equals(CellType.String))
                        {
                            ...
                            foreach (Match match in matches)
                            {
                                dic.AddParameter(new Parameter(sheet.SheetName, match.Value, new Point(cell.RowIndex, cell.ColumnIndex)));
                            }
                        }
                    }
                }
